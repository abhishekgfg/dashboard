...
const StudentsWithoutSeats = () => {
  ...
  const students = [
    { id: 'S1044', name: 'Advik Sharma', image: 'path/to/advik.jpg', imsId: '_6857091274bc' },
  ...
  return (
    <div className="no-seat-container">
  <h2 className="no-seat-title">Students Without Allotted Seats</h2>
  <div className="no-seat-grid">
    {students.map((student, index) => (
      <div key={index} className="no-seat-card">
        {student.image ? (
          <img src={student.image} alt={`${student.name} photo`} className="no-seat-image" />
        ) : (
          <div className="no-seat-placeholder">
        ...
        <button className="no-seat-button">Allot Seat</button>
      </div>
    ))}
  </div>
